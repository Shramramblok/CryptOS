...
Disk::Disk(uint8_t driveNumber) {
    ...
    if (!x86_GetDiskParamsProt(driveNumber, &this->cylinders, &this->heads, &this->sectors, &this->driveType))
    {
...
bool Disk::read(uint32_t lba, uint8_t* buffer, uint8_t count) {
    ...
    {
        if (x86_ReadDiskProt(this->driveNumber, cylinder, head, sector, count, buffer))
            return true;
        x86_ResetDiskProt(this->driveNumber);
    }
    return false;
}
